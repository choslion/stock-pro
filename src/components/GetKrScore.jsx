...
function getMeta(score) {
  if (score <= 20) return { label: "극단적 공포", advice: "강력한 매수 기회",        explanation: "국내 시장이 과도하게 하락한 상태" };
...
export default function GetKrScore() {
  ...
  if (loading) {
    return (
      <Card title="국내 시장 심리" subtitle="KOSPI 변동성 · 추세 · 업종폭 기반" icon={ActivityIcon}>
        <Spin />
      </Card>
    );
  }
      ...
      {detail && (
        <div className="mt-4 grid grid-cols-3 gap-2 border-t border-gray-700 pt-3">
          <div className="text-center">
            ...
            <p className="text-sm font-semibold text-orange-300 mt-0.5">{detail.realized_vol}%</p>
          ...
          <div className="text-center">
            ...
            <p className="text-sm font-semibold text-green-400 mt-0.5">{detail.breadth_pct}%</p>
