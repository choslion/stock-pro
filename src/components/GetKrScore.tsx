...
function getMeta(score: number): ScoreMeta {
...
export default function GetKrScore() {
  ...
  const score  = target;
  const meta   = data ? getMeta(score) : null;
  const detail = data ?? null;

  if (isLoading) {
    return (
      <Card title="지금 투자 타이밍인가요?" subtitle="KOSPI 변동성 · 추세 · 업종폭 기반" icon={ActivityIcon}>
        <Spin />
      </Card>
    );
  }
