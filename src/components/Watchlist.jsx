...
const KR_TICKERS = WATCHLIST.filter((w) => w.market === "KR").map(
  (w) => w.ticker,
);
...
export default function Watchlist() {
  ...
  useEffect(() => {
    ...
    setError("");
    const params = {};
    if (KR_TICKERS.length) params.kr = KR_TICKERS.join(",");
            ...
            <div className="divide-y divide-gray-700/50">
              {WATCHLIST.map((config) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`col-span-1 text-xs font-semibold ${config.market === "KR" ? "text-blue-400" : "text-yellow-400"}`}
                    >
                      {config.market}
                    </span>
