...
import type { IndexData } from "../types/api";
...
function colorClass(changePct: number) {
  return changePct >= 0 ? "text-red-400" : "text-blue-400";
}

interface IndexRowProps { label: string; data?: IndexData; }
interface IndexBlockProps { label: string; data?: IndexData; border?: boolean; }

/* ── 모바일: 가로 한 줄 (라벨 좌 | 값 우) ── */
function IndexRow({ label, data }: IndexRowProps) {
  if (!data) return null;
