...
export default function StockRanking() {
  ...
  useEffect(() => {
    setLoading(true);
    setError("");
    const url = market === "overseas" ? "/stocks/us-ranking" : "/stocks/ranking";
    axiosInstance
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between border-b border-gray-700 mb-3">
        <div className="flex gap-1">
          {MARKETS.map((m) => (
            <button
              key={m.id}
              onClick={() => { setMarket(m.id); setFilter("amount"); setCurrency("krw"); }}
      ...
      <div className="flex items-center justify-between mb-2 min-h-[16px]">
        {isOverseas && usdKrw && (
          <p className="text-xs text-gray-500">1 USD = {usdKrw.toLocaleString("ko-KR")}원</p>
        )}
        {fetchedAt && (
