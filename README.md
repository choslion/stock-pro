...
- **시장 데이터 통합** - 국내·미국 지수, VIX·공포탐욕지수, 섹터, ETF, 환율·원자재와 증시 뉴스를 탭별로 조회하고, KRX 서킷브레이커·사이드카 발동 시 전역 알림을 표시합니다.
- **종목 탐색과 관심종목** - 국내외 종목 검색, 가격 차트, 거래 순위를 제공하며 개인 관심종목은 브라우저에 최대 30개까지 저장합니다.
- **모의 포트폴리오** - 가상 자금 1,000만 원으로 국내외 종목을 사고팝니다. 이동평균법으로 평단과 실현손익을 계산하고, 같은 날 같은 금액을 지수에 넣고 뺐다면 어땠을지를 KOSPI·S&P 500과 비교해 보여줍니다.
...
- 검색·탭 전환·공통 UI와 투자 시뮬레이션·포트폴리오 계산 로직을 **9개 테스트 파일, 67개 테스트 케이스**로 검증하며 GitHub Actions에서 타입 검사·테스트·빌드를 실행합니다.
